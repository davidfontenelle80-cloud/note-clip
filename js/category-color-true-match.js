/* category-color-true-match.js — Stage 16E clean color picker/save */
(function(App){
  'use strict';
  ...
  function currentCatId(){
    var save=[].slice.call(document.querySelectorAll('#cat-modal button')).find(function(b){return (b.getAttribute('onclick')||'').indexOf('_saveCat')>-1;});
    var m=(save&&save.getAttribute('onclick')||'').match(/_saveCat\('([^']*)'\)/);
    return m?m[1]:'';
  }
  function currentColor(){
    var id=currentCatId();
    var cat=id && App.Storage && App.Storage.getState().categories.find(function(c){return c.id===id;});
  ...
  function hideOldAccent(){
    document.querySelectorAll('#cat-modal .cat-accent-preview,#cat-modal .cat-accent-grid').forEach(function(el){
      var group=el.closest('.form-group')||el;
      group.style.display='none';
    });
    document.querySelectorAll('#cat-modal label,.modal-title').forEach(function(el){
      if(/accent/i.test(el.textContent||'')){var g=el.closest('.form-group')||el;g.style.display='none';}
    });
  }
  function inject(){
    ...
    if(!modal.querySelector('[data-cat-color-section]')){
      ...
      if(group)group.insertAdjacentHTML('afterend',html(currentColor()));
    }
    ...
    var save=[].slice.call(modal.querySelectorAll('button')).find(function(b){return (b.getAttribute('onclick')||'').indexOf('_saveCat')>-1;});
    if(save&&!save.dataset.colorSaveReady){
      save.dataset.colorSaveReady='1';
      save.onclick=function(ev){
        ...
        var color=(document.getElementById('cat-color')&&document.getElementById('cat-color').value)||'#FFF4B8';
        if(!name){App.showToast&&App.showToast('Category name required','error');return;}
